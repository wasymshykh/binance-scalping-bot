/*
    * author: @kroitor (https://github.com/kroitor)
    * modified by: @wasymshykh (https://github.com/wasymshykh)
*/
...
(function (exports) {
    ...
    exports.plot = function (series, cfg = undefined) {
        ...
        for (let j = 0; j < series.length; j++) {
            ...
            for (let x = 0; x < series[j].length - 1; x++) { // plot the line
                ...
                if (y0 == y1) {
                    ...
                    if (x === series[j].length-2) {
                        last_column.push(rows - y0);
                        result[rows - y0][x + offset] = colored((symbols[4]+'→'+(series[j][x + 1])), currentColor)
                    } else {
                    ...
                    if (x === series[j].length-2) {
                        ...
                        result[rows - y1][x + offset] = colored(((y0 > y1) ? symbols[5] : symbols[6])+'→'+(series[j][x + 1]), currentColor)
                        
                        last_column.push(rows - y1);
                        last_line_column.push(rows - y0);
        ...
        // fix old remaining numbers on screen (when chart moves) 
        result.forEach((r, i) => {
            if (!last_column.includes(i)) {
                if (last_line_column.includes(i)) {
                    r[r.length - 2] += ' '.repeat(6);
                } else {
                    r[r.length - 2] = ' '.repeat(7);
                }
            }
        })
